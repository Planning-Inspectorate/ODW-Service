...
class SynapseDatabaseTableUtil(SynapseArtifactUtil):
    """
    ...
    def get_all(self, **kwargs: Dict[str, Any]) -> List[Dict[str, Any]]:
        response = self._web_request(
            f"{self.synapse_endpoint}/databases/logging/tables?api-version=2020-12-01",
        ).json()
    ...
    def create(self, db_name: str, table_name: str, table_json: Dict[str, Any]):
        api_call_headers = {'Authorization': 'Bearer ' + self._token}
        resp = requests.put(
            f"{self.synapse_endpoint}/databases/{db_name}/tables/{table_name}?api-version=2020-12-01",
...
dbs = SynapseDatabaseTableUtil("pins-synw-odw-prod-uks").get_all()
