...
function Get-GatewayBinary {
  Write-Host "Checking for existing Microsoft Integration Runtime binary"
  $MsiFiles = (Get-ChildItem -Path 'C:\SHIR' | Where-Object { $_.Name -match [regex] "IntegrationRuntime.*.msi" })

  If ($MsiFiles) {
    $MsiFileName = $MsiFiles[0].Name
    Write-Host "Found existing Microsoft Integration Runtime binary: $MsiFileName"
